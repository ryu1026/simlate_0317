...
move_image = shift_beads_matrix(image, 170)
...
x_random = [500, 340, 360, 400]
y_random = [500, 350, 410, 240]
x_triangle_center = [400, 400, 430, 430,  460, 440, 440]
y_triangle_center = [240, 270, 260, 290, 280, 270, 270]
...
for j in range(len(x_triangle) - 1 ):
    ax.plot([x_triangle[j], x_triangle[j+1]], [y_triangle[j], y_triangle[j+1]], marker='o', markersize=8, color="orange")
    if j == len(x_triangle) - 2:
        ax.plot([x_triangle[j+1]], [y_triangle[j+1]], marker='o', markersize=2, color='orange')

# 3点計測の中心の描画
for j in range(len(x_triangle_center) - 1):
    ax.plot([x_triangle_center[j], x_triangle_center[j+1]], [y_triangle_center[j], y_triangle_center[j+1]], marker='o',
